...
public interface ReportFieldMapper {
    ...
    @Select("SELECT id,report_id,label,field_name,field_type,match_type,searchable,search_sort,default_query_days,max_query_days,sort_order,deleted,created_at,updated_at " +
            "FROM custom_report_field WHERE report_id=#{reportId} AND deleted=0 ORDER BY sort_order ASC")
    List<ReportFieldEntity> findByReportId(@Param("reportId") Long reportId);

    @Select("SELECT COUNT(1) FROM custom_report_field WHERE report_id=#{reportId} AND deleted=0")
    long countByReportId(@Param("reportId") Long reportId);

    @Insert("INSERT INTO custom_report_field(id,report_id,label,field_name,field_type,match_type,searchable,search_sort,default_query_days,max_query_days,sort_order,deleted,created_at,updated_at) " +
    ...
    @Update("UPDATE custom_report_field SET deleted=1,updated_at=NOW() WHERE report_id=#{reportId} AND deleted=0")

...
public interface ReportConfigMapper {
    ...
    @Select("<script>" +
            "SELECT id,data_source_id,name,router_path,procedure_name,page_size,exporters,export_wait_message,query_enabled,download_enabled,deleted,created_at,updated_at " +
            "FROM report_config " +
            "WHERE deleted=0 " +
    ...
    @Select("SELECT id,data_source_id,name,router_path,procedure_name,page_size,exporters,export_wait_message,query_enabled,download_enabled,deleted,created_at,updated_at " +
    ...
    @Insert("INSERT INTO report_config(id,data_source_id,name,router_path,procedure_name,page_size,exporters,export_wait_message,query_enabled,download_enabled,deleted,created_at,updated_at) " +
            "VALUES(#{id},#{dataSourceId},#{name},#{routerPath},#{procedureName},#{pageSize},#{exporters},#{exportWaitMessage},#{queryEnabled},#{downloadEnabled},0,NOW(),NOW())")
    int insert(ReportConfigEntity entity);

    @Update("UPDATE report_config SET data_source_id=#{dataSourceId},name=#{name},router_path=#{routerPath},procedure_name=#{procedureName},page_size=#{pageSize}," +
